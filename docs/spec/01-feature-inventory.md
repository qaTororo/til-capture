...
til-capture の全機能を一覧化し、バージョンごとの優先度を管理する。
...
| F-002 | ストック表示 + Author 表示 | SessionStart hook | `hooks/session-start-hook.sh` | `[Implemented]` | [03-auto-capture.md](./03-auto-capture.md) |
...
- **テスト**: 25 テストケース（`test/stop-hook.bats`）
...
- **テスト**: 17 テストケース（`test/session-start-hook.bats`）
...
CWD → config.json の優先順位で保存先を決定する。未設定時はアラート表示。

- **優先順位**: `src/content/til/` > `content/til/` > `til/`（CWD内） → `config.json`
- **設定ファイル**: `~/.config/til-capture/config.json`
- **未設定時**: アラート表示（保存しない）— [ADR-004](../adr/ADR-004-directory-resolution-strategy.md)
...
|----|--------|------|---------|--------|------|-----------|------|
| F-102 | タグ自動補完 | UX 改善 | **v1.0** | 中 | F-003 | `[Implemented]` | [06-future-features.md](./06-future-features.md#f-102-タグ自動補完) |
| F-107 | TIL 検索・一覧表示 | Skill 追加 | **v1.0** | 低 | F-004 | `[Implemented]` | [06-future-features.md](./06-future-features.md#f-107-til-検索一覧表示) |
...
graph TD
    ...
    style F102 fill:#4CAF50,color:#fff
    style F107 fill:#4CAF50,color:#fff
    style F111 fill:#4CAF50,color:#fff
    style F101 fill:#FFC107,color:#000
    style F103 fill:#9E9E9E,color:#fff
```

**凡例**: 緑 = Implemented / 黄 = Draft / グレー = Out of Scope

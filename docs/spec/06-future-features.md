...
v1.0 で実装を検討する候補機能を Draft ステータスで記載する。各候補は [01-feature-inventory.md](./01-feature-inventory.md) の優先度マトリクスに基づき、優先実装 → 次フェーズ → 余裕があれば の順で整理している。
...
> **設計ノート**: 現在 til-capture が使用している Hook イベントは `Stop` と `SessionStart` のみ。新規 Hook イベントの活用は Claude Code Plugin の仕様拡張に依存するため、仕様が確定してから詳細設計を行う。

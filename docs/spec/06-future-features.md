...
- チーム運用では `author` フィールドを追加したい → **v1.1 で config.json トップレベルに実装済み**
...
> **ステータス**: `[Out of Scope]` — スコープ外に変更（v1.1 判断）
>
> **除外理由**:
> 1. 個人利用でも重複出現は知識未定着のサインであり、記録する価値がある
> 2. AI にまとめ読みさせるスタイルでは重複は問題にならない
> 3. 頻出することで AI が重み付けする可能性があり、それは許容範囲内

...
### TIL の位置づけ（ADR-002 で確定）

> TIL の本質的な価値は **公開用記事の下書き** ではなく、**個人の知識整理・学びなおしのツール** にある。

AI 生成コンテンツの氾濫により、ユーザーは「AI らしいテキスト」の存在に疲れ始めている。Claude が生成した TIL メモをそのまま公開するのではなく、それを起点に自分自身で知識を再構築し、自分の言葉でアウトプットすることに価値がある。

この位置づけは以下の機能優先順位に影響する:
- **参照効率の向上**（タグ一貫性、検索機能）が最も高い価値
- **SSG 向けカスタマイズ**（テンプレート、エクスポート）は優先度低

### v1.0 ゴール

v0.3 で確立した基本機能（自動検知・手動記録・ストック表示）を土台に、以下を達成する:

1. **参照効率の向上**: タグ自動補完（F-102）と TIL 検索・一覧表示（F-107）による「学びなおし」の支援
2. **UX の洗練**: 信頼度ベースの確認フロー（ADR-001）、保存先の意図的な設定の要求（ADR-004）
3. **設計方針の確立**: 新規 Hook イベントへの慎重待機（ADR-003）
...
- frontmatter 付き Markdown（様々なツールと互換性がある）
- ファイル名規則: `YYYY-MM-DD-<slug>.md`
- ディレクトリ構造に依存しない（保存先は設定で変更可能）

> **補足**: SSG（Hugo, Astro 等）との互換性は副次的な利点。v1.0 の主要な価値は「個人の知識整理」にある（[ADR-002](../adr/ADR-002-v1-feature-priority.md)）。
...
### セカンダリ: チーム内ナレッジ共有担当（v1.1+ で対応予定）

> **注**: v1.0 のスコープは Aさんに集中しました。v1.1 で `author` フィールドを追加し、チーム利用の基礎を対応しました。SSG 連携（テンプレートカスタマイズ F-101）は引き続き将来の検討対象です（[ADR-002](../adr/ADR-002-v1-feature-priority.md)）。
...
```
1. 過去の TIL を検索・一覧表示（F-107 で強化予定）
2. タグで分類された TIL をブラウズ（F-102 でタグ一貫性を向上）
3. TIL を起点に自分の言葉で知識を再構築・アウトプット
```

> **補足**: SSG でのブログ公開やチーム共有は v1.1+ での対応予定です（[ADR-002](../adr/ADR-002-v1-feature-priority.md)）。
